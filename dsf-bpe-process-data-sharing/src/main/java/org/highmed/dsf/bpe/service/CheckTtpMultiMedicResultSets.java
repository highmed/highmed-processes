...
import static org.highmed.dsf.bpe.ConstantsDataSharing.BPMN_EXECUTION_ERROR_CODE_MULTI_MEDIC_DATA_SHARING_RESULT;
...
public class CheckTtpMultiMedicResultSets extends CheckResultSets
{
	...
	protected List<QueryResult> postProcessAllPassingResults(List<QueryResult> passedResults)
	{
		if (!atLeastOneResultExists(passedResults))
			throw new BpmnError(BPMN_EXECUTION_ERROR_CODE_MULTI_MEDIC_DATA_SHARING_RESULT);
	...
	private boolean atLeastOneResultExists(List<QueryResult> results)
	{
		...
		{
			logger.warn(
					"Did not receive any results from participating MeDICs for any cohort definition in the "
							+ "data sharing request with task-id='{}', business-key='{}' and correlation-key='{}'",
			...
			leadingTask.getOutput()
					.add(getTaskHelper().createOutput(CODESYSTEM_HIGHMED_BPMN, CODESYSTEM_HIGHMED_BPMN_VALUE_ERROR,
							"Did not receive any results from participating MeDICs for any cohort definition"));
